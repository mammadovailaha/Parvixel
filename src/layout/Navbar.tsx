  ...
  return (
    <div className="w-full h-20 bg-white dark:bg-gray-900 px-4 py-2 overflow-x-hidden fixed top-0 left-0 z-50 shadow-md">
      <div className="w-[95%] flex justify-between items-center overflow-x-hidden mx-auto">
        ...
        <div
          className={`w-full md:w-[45%] lg:w-[40%] 2xl:w-[30%] block md:flex items-center justify-center 
        gap-5  px-6 py-3.5 rounded-[30px]  absolute  z-[999]  h-[calc(100vh-4rem)] md:static h-full transition-transform duration-300 ease-in-out
    transform
          ...
          <ul className="w-full md:flex items-center justify-around">
            {navLinks.map((link) => (
              <li key={link.to}>
                <Link
                  to={link.to}
                  className="relative inline-block text-black dark:text-white font-medium text-sm lg:text-[17px] tracking-normal overflow-hidden group h-[24px]" // h-[24px] yazırıq ki, textlər üst-üstə qapanmasın
                >
                  {/* Ön tərəf */}
        ...
        <div className="flex justify-center items-center gap-2.5">
          ...
          <div className="md:hidden h-18 lg:h-[86px]  flex justify-end items-center ">
            <button
              ...
              className={`outline-none border-none bg-gray-200 text-white rounded-[30px] px-2 py-2 cursor-pointer ${
                isOpen ? "absolute right-0 z-1001" : ""
              }`}
            >
              {isOpen ? (
                <IoMdClose size={28} className=" text-black " />
              ) : (
                <RiMenu5Fill className="text-xl dark:text-white text-gray-800 " />
              )}
            </button>
          </div>
        </div>
